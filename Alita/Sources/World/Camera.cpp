...
PerspectiveCamera::PerspectiveCamera(float fov, float aspect, float nearPlane, float farPlane)
	...
	, farPlane_(farPlane)
{
	transform_.Position() = TVector3{ 2.0f, 2.0f, 2.0f };
	...
	//viewMatrix_ = glm::lookAtRH(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, -2.0f), glm::vec3(0.0f, 1.0f, 0.0f));

	//// rotate
	//viewMatrix_ = glm::rotate(viewMatrix_, ANGLE_TO_RAD(transform_.Rotation().x), TVector3(1.0f, 0.0f, 0.0f));
	//viewMatrix_ = glm::rotate(viewMatrix_, ANGLE_TO_RAD(transform_.Rotation().y), TVector3(0.0f, 1.0f, 0.0f));
	//viewMatrix_ = glm::rotate(viewMatrix_, ANGLE_TO_RAD(transform_.Rotation().z), TVector3(0.0f, 0.0f, 1.0f));

	// translate to init position
	//viewMatrix_ = glm::translate(viewMatrix_, transform_.Position());

	// right hand projection
	//projMatrix_ = glm::perspectiveRH(glm::radians(fov_), aspect_, nearPlane_, farPlane_);
}
